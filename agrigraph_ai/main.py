    ...
    try:
        ...
        try:
            ...
            if graph.edge_index.shape[1] == 0:
                raise RuntimeError("Graph construction produced empty graph (no edges)")
        ...
        try:
            ...
            print(f"  ✓ Model initialized: {Config.INPUT_FEATURES} -> {args.hidden_dim} (x{args.num_layers}) -> {Config.OUTPUT_DIM}")
        except Exception as e:
        ...
        if not args.skip_training:
            ...
            try:
                if args.model_path:
                    ...
                    print("  ⚠ Skipping training but no model path provided, using initial model")
                trained_model = model
                history = None
                test_metrics = None
            except Exception as e:
        ...
        try:
            ...
            raise RuntimeError(f"Prediction generation failed: {e}")
        ...
        try:
            ...
            if high_risk_alerts or critical_alerts:
                ...
                print(f"  ✓ {len(urgent_alerts)} urgent alerts saved")
